/*
//tout marche sauf le set rand color qui ne passe pas la loc de l'image aleatoire a la cell 
...
/*
 TODO definir tous les bonbons une fois dans Level, faire passer leur adresse dans cell -> rectangle (vercteur<b_rouge, b_vert, etc)
 comme ça on les initialise une seule fois et on ne fait pas trop de new + peut etre faire le switchcase initial de couleur dans Level
...
class Button {
	...
	public:
		...
		Button(Point center, const char* label, int w, int h, ButtonType type):
...
class Rectangle {
 ...
 public:
  ...
  void setPng(Fl_PNG_Image *png) {png_img = png; img_box->image(png);}
...
The Level class below will have cells as instance
...
class Cell {
	...
	Point getN_Center() {return {(colonne * 100) + 50, 100 + (ligne * 100) + 50};}
...
class Animation {
	...
	public:
		enum AnimationType{swap, explode, fall};
	...
	public:
		...
		Animation(Cell* cellToAnimate, int leap,  AnimationType animationType, int speed):
		base {cellToAnimate}
		{
			coord_base = base->getRect().getCenter();
			max = leap * 100;
...
void Animation::draw(){
	...
	if(animationType == swap){
		...
		base->getRect().getImageBonbon().box->position(currentTranslation().x - 50, currentTranslation().y - 50);
		base->getRect().setCenter({currentTranslation().x, currentTranslation().y });
		base->drawWithoutAnimation();
	}
}
...
Point Animation::currentTranslation(){
	...
	{
		return {coord_base.x , coord_base.y + time};
	}
...
bool Animation::isComplete(){
	if(time > max){
		if(animationType == swap){
			...
			base->getRect().setPng(blank.locImg);
		}else if (animationType == fall)
		{
			base->getRect().getImageBonbon().box->position(coord_base.x - 50, coord_base.y - 50 + max);
			base->getRect().setCenter({coord_base.x, coord_base.y + max});	
		}
...
One instance of the Level class is made by the
MainWindow class.
...
class Level {
 ...
 public:
  ...
  void switchCells(CTS cts, int speed, bool anim);
  void checkNeighbors();
...
bool Level::zeroTop(){
	for (auto &v: cells)
		for (auto &c: v){
			Fl_PNG_Image * cImg = c.getRect().getImageBonbon().png;
			
			if (cImg == images.blank() && c.getX() != 0 && cells[c.getX() -1 ][c.getY()].getRect().getImageBonbon().png != images.blank())
			{
				return False;
			}
		}
...
void Level::initLevel(int levelNum) {
  initBG(levelNum);
  string niveau;
  lvlpath = ("niveaux/n%d/%d", levelNum, levelNum);
	highpath = "niveaux/n1/h1";
  int b_type, id, elem;
	file.open(lvlpath);
...
void Level::initBG(int levelNum){
	bgpath = "Images/Misc/background1.png";
	background.box = new Fl_Box(0, 0, 900, 1000);
	background.png = new Fl_PNG_Image(bgpath.c_str());
...
void Level::draw() {
	...
	if(!isAnimated() && !isBlank()){
		...
		swapUP();	
	}else if (!isAnimated() && zeroTop() && isBlank()){
		toSwap.clear();
		setrandcolor();
	}
...
void Level::mouseClick(Point mouseLoc) {
    for (auto &v: cells){
        for (auto &c: v){
...
void Level::mouseMove(Point mouseLoc) {
  for (auto &v: cells)
    for (auto &c: v)
      c.mouseMove(mouseLoc);
}

void Level::switchCells(CTS cts, int speed, bool anim){
	...
	if (anim){
		Animation *a = new Animation(&cells[cts.coord_1.x][cts.coord_1.y], &cells[cts.coord_2.x][cts.coord_2.y], static_cast<Animation::AnimationType>(0),speed);
		cells[cts.coord_2.x][cts.coord_2.y].setAnimation(a);
		Animation *aa = new Animation(&cells[cts.coord_2.x][cts.coord_2.y], &cells[cts.coord_1.x][cts.coord_1.y], static_cast<Animation::AnimationType>(0),speed);
...
void Level::setrandcolor(){
	if(!isAnimated() && toSwap.size() == 0){
		...
		for(auto &v : cells){
			for (auto &c : v){
				if (c.getRect().getImageBonbon().png == images.blank()){
					...
					c.getRect().setPng(images.getImginf(randColor).locImg); 
					Cell c_ = {{c.getRect().getCenter().x, c.getRect().getCenter().y}, 100, 100, images.getImginf(1), c.getId(), c.getX(), c.getY()};
					c.getRect().getImageBonbon().box->position(c.getRect().getImageBonbon().box->x(), c.getRect().getImageBonbon().box->y() - (200 + (100 * max_X)));
					c.getRect().setCenter({c.getRect().getCenter().x, c.getRect().getCenter().y - (200 + (100 * max_X))});
					Animation *a = new Animation(&c, &c_, static_cast<Animation::AnimationType>(0), 12);
					c.setAnimation(a);
...
void Level::swapUP(){
	...
	for(auto &v : cells){
		for (auto &c : v){
			if(c.getRect().getImageBonbon().png == images.blank()){
				int x_now = c.getX();
					if (x_now > 0 && cells[x_now -1 ][c.getY()].getRect().getImageBonbon().png != images.blank()){
						x_now--;
						CTS cts = {cells[x_now+1][c.getY()].getN_Center(), cells[x_now][c.getY()].getN_Center(),
...
/*
void Level::swapUP(){
	leap = toSwap.back().x - toSwap.front().x;
	fallGuys(leap);
	for(auto &v : cells){
		for (auto &c : v){
			if(c.getRect().getImageBonbon().png == images.blank()){
...
void Level::fallGuys(int leap){
	vector<Point> topPoufs;
	vector<Cell&> guys;
	for (auto coord : toSwap){
		for (auto top : topPoufs){
			if (coord.y == top.y && coord.x > top.x){
				remove(topPoufs.begin(), topPoufs.end(), top);
				topPoufs.push_back(coord);
			}
			else{
				topPoufs.push_back(coord);
			}	
	...
	for(auto &v : cells){
		for (auto &c : v){
			for (auto &top : topPoufs){
				if (c.getCoord().y > top.y)
				{
					guys.push_back(c);
					Animation *a = new Animation(&c, leap, static_cast<Animation::AnimationType>(2),8);
					c.setAnimation(a);
				}
			}	
		}
...
void Level::pouf(Recurrence recurrence){
...
void Level::addscore(int longeur){
    score = score + (100 * longeur);
    if (score > highscore)
    {
      highscore = score;  
      file.open(highpath);
      ofstream temp(highpath);
      temp << highscore;
      temp.rdbuf();
      temp.close();
      file.close();
...
class Menu {
	...
	Menu();
	void init();
	bool getGamestate(){return inLevel;}
	void init_selector();
	void draw();
...
void Menu::init(){
	...
	img_png = new Fl_PNG_Image("Images/Misc/back_menu.png");
	img_box->image(img_png);
	const char* label_1 = "Sélecteur de niveau";
	const char* label_2 = "Modificateur de niveau";

	btnList.push_back({{150, 195}, label_1, 200, 50, static_cast<Button::ButtonType>(0)});
	btnList.push_back({{150, 255}, label_2, 200, 50, static_cast<Button::ButtonType>(1)});
}

void Menu::test(Fl_Button* obj, void*){
	cout << "test" << endl;
}
...
void Menu::mouseMove(Point p) {
	
}
void Menu::mouseClick(Point p) {
...
class MainWindow : public Fl_Window {
 ...
 public:
  ...
  int handle(int event) override {
	  bool gamestate = menu.getGamestate();
    switch (event) {

      case FL_MOVE:
		if (gamestate){
			menu.getLevel()->mouseMove(Point{Fl::event_x(), Fl::event_y()});
		}else{
      ...
      case FL_PUSH:
	  	if (gamestate){
		  menu.getLevel()->mouseClick(Point{Fl::event_x(), Fl::event_y()});
		}else{
      ...
      case FL_KEYDOWN:
	  if (gamestate){
		  menu.getLevel()->keyPressed(Fl::event_key());
	  }else{
		  menu.keyPressed(Fl::event_key());
	  }
        return 1;
    }
    return 0;
